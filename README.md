...
Display language defaults to English. Open the extension settings panel with `/ars-setting`, select a language, then press `Ctrl+S` to save and reload. / 顯示語言預設英文；可用 `/ars-setting` 開啟設定面板，選擇語言後按 `Ctrl+S` 儲存並 reload。

To show Traditional Chinese command descriptions from startup / 若要啟動時直接顯示繁中指令描述：

```bash
PI_IEEE_LANG=zh-TW pi -e .
```

Or create `.pi/ieee-academic-research-ieee.json` in the project using this package / 或在使用本套件的專案建立設定檔：
